...
module.exports = function (Categories) {
  Categories.create = async function (data) {
    ...
    await db.sortedSetAddBulk([['categories:cid', result.category.order, result.category.cid], [`cid:${parentCid}:children`, result.category.order, result.category.cid], ['categories:name', 0, `${data.name.slice(0, 200).toLowerCase()}:${result.category.cid}`], ['categoryhandle:cid', cid, handle]]);
